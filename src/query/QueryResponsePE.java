...
public class QueryResponsePE
{
	...
	//for simple stat
	private String gameMode, mapName,whitelist,ip;
	...
	public QueryResponsePE(byte[] data, boolean fullstat)
	{
...
//		if(temp.length == 6) //short stat
		...
		else //full stat
		{
			...
			whitelist = new String(temp[23]);
			
			int start=25;byte[] d;
			while((d=temp[start++]).length==0?false:d[0]!=1)if(start>=temp.length)return;
			start++;
